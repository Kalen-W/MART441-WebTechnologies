...
class playerCharacter {
  constructor() {
    ...
    this.sb = 0.7;      // Base Speed
    this.asb = Math.round(Math.sqrt((this.sb * this.sb) / 2) * 1000) / 1000; // angle speed base
    this.s = this.sb;   // Speed
    this.sBlt = 0.9;    // Bullet Speed
    this.h = 4;         // Health
    this.it = 125;      // Invincibility Time (in milliseconds)
  ...
  movement(timeDelta = 1) {
    ...
    if ((keyIsDown(this.le1) || keyIsDown(this.le2)) && this.x > 32) {
      this.x -= this.s * timeDelta;
    }
    if ((keyIsDown(this.ri1) || keyIsDown(this.ri2)) && this.x < screenW-32) {
      this.x += this.s * timeDelta;
    }
    if ((keyIsDown(this.up1) || keyIsDown(this.up2)) && this.y > 32) {
      this.y -= this.s * timeDelta;
    }
    if ((keyIsDown(this.do1) || keyIsDown(this.do2)) && this.y < screenH+16) {
      this.y += this.s * timeDelta;
    }
  }
  ...
  shoot() {
    ...
    if (player.gLvl == 2) {
      bullets.push(new projectile(this, this.dmg, this.sBlt, 38,18));
      bullets.push(new projectile(this, this.dmg, this.sBlt, 0,-20));
      bullets.push(new projectile(this, this.dmg, this.sBlt, -38,18));
    } else if (player.gLvl == 1) {
      bullets.push(new projectile(this, this.dmg, this.sBlt, 22,-12));
      bullets.push(new projectile(this, this.dmg, this.sBlt, -22,-12));
    } else {
      bullets.push(new projectile(this, this.dmg, this.sBlt, 0,-32));
    }
  }
