...
var colors = {
  background: "#262628",
  bgOpacity: "ff",
  triangleColor: "#6098aa",
  circleColor: "#24b6d6"
};

var points = [];
//var pointAmount = 64;
var pointAmount = Math.floor((screenW*screenH)/15800);
var currentShape = "circle";


//generatePointsArray(currentShape);
//var animationInterval = setInterval(animationLoop, tick);
...
function circle(xCenter, yCenter, radius, opacity) {
  context.fillStyle = colors.circleColor + toHex(opacity);
  context.beginPath();
...
function triangle(xCenter, yCenter, radius, opacity, rotation)  {
  context.fillStyle = colors.triangleColor + toHex(opacity);
  rotation *= Math.PI;
...
function floatPoints(shape) {
  points.forEach((item, i) => {
    if (Math.random()*.9 > Math.random()*8) {
...
function windowSizeChange() {
  ...
  pointAmount = Math.floor((screenW*screenH)/15800);
  generatePointsArray(currentShape);
}
